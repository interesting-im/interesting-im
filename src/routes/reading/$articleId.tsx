    ...
    <div className="min-h-screen bg-neutral-900 flex flex-col">
      ...
      {showSpread ? (
        ...
        <div className="flex-1 flex">
          ...
          <div className="hidden xl:flex flex-col w-36 bg-neutral-800/20 p-3 border-r border-neutral-700">
            <div className="sticky top-24">
              ...
              <div className="space-y-1 max-h-60 overflow-y-auto">
                {currentPageVocab.length > 0 ? (
                  ...
                  <p className="text-neutral-500 text-xs">No vocab</p>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Left Page Content */}
          <div className="flex-1 p-8 lg:p-12 border-r border-neutral-700 flex flex-col">
            ...
            <div className="bg-neutral-800 border-t border-neutral-700 p-3">
              {selectedWordLeft ? (
                <div>
                  <span className="text-base font-semibold text-neutral-100">{selectedWordLeft.word}</span>
                  {selectedWordLeft.pronunciation && <span className="text-neutral-400 ml-2 text-sm">{selectedWordLeft.pronunciation}</span>}
                  <p className="text-neutral-300 text-sm mt-0.5">{selectedWordLeft.translation}</p>
                </div>
              ) : (
          ...
          <div className="flex-1 p-8 lg:p-12 flex flex-col">
            ...
            <div className="bg-neutral-800 border-t border-neutral-700 p-3">
              {selectedWordRight ? (
                <div>
                  <span className="text-base font-semibold text-neutral-100">{selectedWordRight.word}</span>
                  {selectedWordRight.pronunciation && <span className="text-neutral-400 ml-2 text-sm">{selectedWordRight.pronunciation}</span>}
                  <p className="text-neutral-300 text-sm mt-0.5">{selectedWordRight.translation}</p>
                </div>
              ) : (
                <p className="text-neutral-500 text-sm text-center">
          ...
          <div className="hidden xl:flex flex-col w-36 bg-neutral-800/20 p-3 border-l border-neutral-700">
            <div className="sticky top-24">
              <h3 className="text-sm font-semibold text-neutral-400 mb-2 flex items-center gap-2">
                <BookOpen className="w-4 h-4" />
                Page {currentPage + 1}
        ...
        <div className="flex-1 flex">
          ...
          <div className="hidden lg:flex flex-col w-20 bg-neutral-800/20 p-4">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="bg-neutral-800 border-t border-neutral-700 p-4">
              <div className="max-w-5xl mx-auto">
                {selectedWordLeft ? (
                  <div>
                    <span className="text-lg font-semibold text-neutral-100">{selectedWordLeft.word}</span>
                    {selectedWordLeft.pronunciation && <span className="text-neutral-400 ml-2">{selectedWordLeft.pronunciation}</span>}
                    {selectedWordLeft.part_of_speech && <span className="text-xs px-2 py-0.5 bg-neutral-700 rounded ml-2 text-neutral-300">{selectedWordLeft.part_of_speech}</span>}
                    <p className="text-neutral-300 mt-1">{selectedWordLeft.translation}</p>
                  </div>
                ) : (
              ...
              <div className="max-w-5xl mx-auto flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  {Array.from({ length: contentPages }, (_, i) => i + 1).map((i) => (
                    <button key={i} onClick={() => { setCurrentPage(i); setSelectedWordIndexLeft(-1); setSelectedWordIndexRight(-1); }} className={clsx("w-2 h-2 rounded-full", i === currentPage ? "bg-amber-400 w-6" : "bg-neutral-600 hover:bg-neutral-500")} />
                  ))}
                </div>
          ...
          {currentPage > 0 && (
            <div className="hidden lg:flex flex-col w-44 bg-neutral-800/20 p-4 border-l border-neutral-700">
              <div className="sticky top-24">
                ...
                <div className="space-y-2">
                  {currentPageVocab.length > 0 ? (
                    ...
                    <p className="text-neutral-500 text-sm text-center py-4">No vocabulary</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {currentPage > 0 && currentPageVocab.length > 0 && (
        <div className="lg:hidden bg-neutral-800/50 border-t border-neutral-700 p-3">
          <div className="flex flex-wrap gap-2 justify-center">
            {currentPageVocab.map((v, index) => (
              <button key={v.id} onClick={() => setSelectedWordIndexLeft(selectedWordIndexLeft === index ? -1 : index)} className={clsx("px-3 py-1.5 rounded-full text-sm", selectedWordIndexLeft === index ? "bg-amber-400 text-amber-900 font-medium" : "bg-neutral-700 text-neutral-300")}>
                {v.word}
              </button>
            ))}
          </div>
